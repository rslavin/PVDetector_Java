...
public class PhraseTree {
     ...
     * Note: this is bad because it's possible to have orphan nodes. -Rocky
     *
     * @param equivTable
     * @return
     * @throws MultiOrNoRootException
...
//            if (t.getParents().size() == 0) {
            if (t.value.toLowerCase().equals("information") || t.value.equals("owl:Thing")) { // TODO this is a temporary fix that only looks for a root node called "thing"
                roots.add(t);
            }
        }
        if (roots.size() != 1) {
